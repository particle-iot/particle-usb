...
export const CloudDevice = base => class extends base {
   ...
   */
  setClaimCode(code) {
    return this.sendRequest(RequestType.SET_CLAIM_CODE, {
      code: code
    });
  }
   ...
   */
  isClaimed() {
    return this.sendRequest(RequestType.IS_CLAIMED).then(rep => rep.claimed);
  }
  ...
  setServerAddress(address, port, protocol) {
    return this._getServerProtocol(protocol).then(protocol => {
      return this.sendRequest(RequestType.SET_SERVER_ADDRESS, {
        protocol: protocol,
        address: address,
  ...
  getServerAddress(protocol) {
    return this._getServerProtocol(protocol).then(protocol => {
      return this.sendRequest(RequestType.GET_SERVER_ADDRESS, {
        protocol: protocol
      });
    });
  }
  ...
  setServerProtocol(protocol) {
    return this.sendRequest(RequestType.SET_SERVER_PROTOCOL, {
  ...
  _setSecurityKey(type, data) {
    return this.sendRequest(RequestType.SET_SECURITY_KEY, { type: type, data: data });
  }

  _getSecurityKey(type) {
    return this.sendRequest(RequestType.GET_SECURITY_KEY, { type: type }).then(rep => rep.data);
  }
  ...
  _getServerProtocol(protocol) {
    ...
    return this.sendRequest(RequestType.GET_SERVER_PROTOCOL).then(rep => rep.protocol);
  }
}
